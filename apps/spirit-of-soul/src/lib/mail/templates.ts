...
/**
 * HTML-Mailvorlagen im Spirit-of-Soul-Theme — helles Creme/Gold, Serife für
 * Überschriften (spiegelt den "The Finest Of Black Music"-Auftritt der
 * Website).
 *
...
const COLORS = {
  ...
  textDim: "#4a4640",
  textMuted: "#8a857c",
  border: "rgba(200,165,106,0.35)",
  hairline: "rgba(25,23,20,0.08)",
};
          ...
          <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px; width:100%; background:${COLORS.surface}; border:1px solid ${COLORS.border}; border-radius:4px;">
            <tr>
              <td style="padding:36px 40px 20px; text-align:center;">
                <div style="font-family:${FONT_BODY}; font-size:13px; letter-spacing:4px; text-transform:uppercase; color:${COLORS.goldDim}; font-weight:600;">Spirit of Soul</div>
                <div style="height:1px; width:48px; background:${COLORS.gold}; margin:16px auto 0; font-size:0; line-height:0;">&nbsp;</div>
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding:22px 40px 32px; border-top:1px solid ${COLORS.hairline};">
                <p style="margin:0; font-family:${FONT_BODY}; font-size:12px; color:${COLORS.textMuted}; line-height:1.7;">
                  Spirit of Soul — The Finest Of Black Music<br />
                  ${escapeHtml(band.email)} · ${escapeHtml(band.location)}<br />
                  <a href="${band.socials.instagram}" style="color:${COLORS.goldDim}; text-decoration:none;">Instagram</a>
                  &nbsp;·&nbsp;
                  <a href="${band.socials.facebook}" style="color:${COLORS.goldDim}; text-decoration:none;">Facebook</a>
                  &nbsp;·&nbsp;
                  <a href="${band.socials.youtube}" style="color:${COLORS.goldDim}; text-decoration:none;">YouTube</a>
                </p>
              </td>
            </tr>
          </table>
...
export function notificationEmailHtml(d: InquiryDetails): string {
  ...
  const rows = [
    ...
    detailRow("Formular", d.formType === "booking" ? "spiritofsoul.com/booking" : "spiritofsoul.com/kontakt"),
  ].join("");
...
export function confirmationEmailHtml(d: InquiryDetails): string {
  const intro =
    d.formType === "booking"
      ? "vielen Dank für Ihre Buchungsanfrage bei Spirit of Soul. Wir haben Ihre Angaben erhalten und melden uns in der Regel innerhalb von 24 Stunden bei Ihnen."
      : "vielen Dank für Ihre Nachricht an Spirit of Soul. Wir melden uns in der Regel innerhalb von 24 Stunden bei Ihnen.";
  ...
  const body = `
    ...
    <p style="margin:0 0 8px; font-family:${FONT_BODY}; font-size:12px; letter-spacing:0.5px; text-transform:uppercase; color:${COLORS.goldDim};">Ihre Angaben</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">${rows}</table>
    ${messageBlock(d.message)}
    <p style="margin:24px 0 0; font-family:${FONT_BODY}; font-size:15px; line-height:1.7; color:${COLORS.textDim};">
      Musikalische Grüße<br />Ihr Spirit of Soul Team
    </p>
  `;

  return emailShell({
    preheader: "Ihre Anfrage bei Spirit of Soul ist eingegangen.",
    heading:
      d.formType === "booking"
