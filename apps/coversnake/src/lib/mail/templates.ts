          ...
          <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px; width:100%; background:${COLORS.surface}; border:1px solid ${COLORS.border};">
            <tr>
              <td style="padding:36px 40px 20px; text-align:center;">
                <div style="font-family:${FONT_MONO}; font-size:12px; letter-spacing:4px; text-transform:uppercase; color:${COLORS.accent};">CoverSnake</div>
            ...
            <tr>
              <td style="padding:22px 40px 32px; border-top:1px solid ${COLORS.hairline};">
                <p style="margin:0; font-family:${FONT_BODY}; font-size:12px; color:${COLORS.textMuted}; line-height:1.7;">
                  CoverSnake — ${escapeHtml(band.claim)}<br />
...
export function confirmationEmailHtml(d: InquiryDetails): string {
  const intro =
    d.formType === "booking"
      ? "vielen Dank für Ihre Buchungsanfrage bei CoverSnake. Wir haben Ihre Angaben erhalten und melden uns in der Regel innerhalb von 24 Stunden bei Ihnen."
      : "vielen Dank für Ihre Nachricht an CoverSnake. Wir melden uns in der Regel innerhalb von 24 Stunden bei Ihnen.";
  ...
  const body = `
    ...
    <p style="margin:24px 0 0; font-family:${FONT_BODY}; font-size:15px; line-height:1.7; color:${COLORS.textDim};">
      Rockige Grüße<br />Ihr Team von CoverSnake
    </p>
  `;

  return emailShell({
    preheader: "Ihre Anfrage bei CoverSnake ist eingegangen.",
    heading:
      d.formType === "booking"
