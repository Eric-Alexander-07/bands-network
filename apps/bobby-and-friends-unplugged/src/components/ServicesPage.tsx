...
export default function ServicesPage({ dbBesetzung, c }: Props) {
  const formations: Array<{ name: string; beschreibung: string | null; eintraege: Array<{ id: string; name: string; beschreibung: string | null }> }> =
    dbBesetzung && dbBesetzung.length > 0
      ...
      : [
          { name: "Kompakte Besetzungen",  beschreibung: "Konzentrierte akustische Atmosphäre für Wohnzimmerkonzerte, Dinner und kleine Empfänge", eintraege: band.formations.small.map((f, i) => ({ id: String(i), name: f.name, beschreibung: f.lineup })) },
          { name: "Erweiterte Besetzungen", beschreibung: "Mehr Groove für größere Feiern, Hochzeiten und Firmenevents",                            eintraege: band.formations.full.map((f, i) => ({ id: String(i), name: f.name, beschreibung: f.lineup })) },
        ];
  return (
    <>
      ...
      <section className="section section-has-rings" style={{ background: "var(--bg-card)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
        <SoundWaves variant="cluster" origin={{ x: 1.04, y: 0.75 }} spread={0.75} seed={2} intensity={0.22} />
        <div className="container">
        ...
        <div className="container">
          <div className="services-split services-split--imgfirst">
            <div className="services-split-img services-split-img--tall" data-animate="fade-up">
              <img src={c.image_main} alt="Bobby & Friends Unplugged auf der Bühne" />
            </div>
            <div>
