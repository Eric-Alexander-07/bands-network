...
export async function POST(request: NextRequest) {
  ...
  try {
    const { error } = await getResend().emails.send({
      ...
      to: band.email,
      replyTo: email,
      subject: `[BOBbastic] ${typeLabel}${details.occasion ? ` – ${details.occasion}` : ""}`,
    ...
    return NextResponse.json(
      {
        ok: false,
        error: `Die Nachricht konnte nicht gesendet werden. Bitte versucht es später erneut oder schreibt uns direkt an ${band.email}.`,
      },
      { status: 502 }
    );
  }
  ...
  try {
    const { error } = await getResend().emails.send({
      ...
      to: email,
      replyTo: band.email,
      subject: "Eure Anfrage bei BOBbastic ist eingegangen",
